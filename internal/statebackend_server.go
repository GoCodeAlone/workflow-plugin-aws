...
//
// Per decisions/0035 (one type carries both concerns), awsIaCServer ALSO
// serves the typed IaC state-backend contract: it persists IaC state via an
// S3IaCStateStore (ported from workflow core) and answers ListBackendNames
// with the single backend name "s3".
//
...
import (
	...
	"github.com/GoCodeAlone/workflow-plugin-aws/internal/statebackend"
	pb "github.com/GoCodeAlone/workflow/plugin/external/proto"
	"google.golang.org/grpc/codes"
	"google.golang.org/grpc/status"
)

// awsStateBackendName is the single iac.state backend name this plugin serves.
const awsStateBackendName = "s3"

// stateBackend holds the lazily-constructed s3 state store plus the guard that
// builds it once. It is embedded into awsIaCServer.
type stateBackend struct {
	mu    sync.Mutex
	store *statebackend.S3IaCStateStore
}
...
func (b *stateBackend) resolveStore() (*statebackend.S3IaCStateStore, error) {
	b.mu.Lock()
	defer b.mu.Unlock()
	if b.store == nil {
		return nil, status.Error(codes.FailedPrecondition,
			"aws state backend: s3 backend is not configured")
	}
	return b.store, nil
}

// setStateStore injects the backing store. Used by the Configure handler and tests.
func (b *stateBackend) setStateStore(s *statebackend.S3IaCStateStore) {
	b.mu.Lock()
	defer b.mu.Unlock()
	b.store = s
}

// s3Config is the iac.state module config the host delivers via the Configure
// RPC. The JSON keys mirror exactly the config keys the in-core spaces/s3
// switch case in workflow core's iac_module.go read.
...
// ── pb.IaCStateBackendServer methods (on awsIaCServer) ──────────────────────

// Configure decodes the host-delivered iac.state module config and lazily
// constructs the s3 state store, satisfying decisions/0036's host-config
...
func (s *awsIaCServer) Configure(_ context.Context, req *pb.ConfigureRequest) (*pb.ConfigureResponse, error) {
	if req.GetBackendName() != awsStateBackendName {
		return nil, status.Errorf(codes.InvalidArgument,
			"aws state backend: Configure for backend %q — this plugin serves only %q",
			req.GetBackendName(), awsStateBackendName)
	}
	var cfg s3Config
	if err := json.Unmarshal(req.GetConfigJson(), &cfg); err != nil {
		return nil, status.Errorf(codes.InvalidArgument,
			"aws state backend: decode Configure config: %v", err)
	}
	if cfg.Bucket == "" {
		return nil, status.Error(codes.InvalidArgument,
			"aws state backend: s3 backend requires 'bucket' config")
	}
	store, err := statebackend.NewS3IaCStateStore(
		cfg.Region, cfg.Bucket, cfg.Prefix, cfg.AccessKey, cfg.SecretKey, cfg.Endpoint)
	if err != nil {
		return nil, status.Errorf(codes.InvalidArgument,
			"aws state backend: construct s3 store: %v", err)
	}
...
func (s *awsIaCServer) GetState(ctx context.Context, req *pb.GetStateRequest) (*pb.GetStateResponse, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("aws state backend: encode GetState response: %w", err)
	}
	return &pb.GetStateResponse{State: pbState, Exists: true}, nil
}

// SaveState inserts or replaces a state record.
func (s *awsIaCServer) SaveState(ctx context.Context, req *pb.SaveStateRequest) (*pb.SaveStateResponse, error) {
	store, err := s.stateBackend.resolveStore()
	if err != nil {
		return nil, err
	}
	...
	if err != nil {
		return nil, fmt.Errorf("aws state backend: decode SaveState request: %w", err)
	}
...
func (s *awsIaCServer) ListStates(ctx context.Context, req *pb.ListStatesRequest) (*pb.ListStatesResponse, error) {
	store, err := s.stateBackend.resolveStore()
	if err != nil {
		return nil, err
	}
	...
	for _, st := range states {
		pbState, err := iacStateToPB(st)
		if err != nil {
			return nil, fmt.Errorf("aws state backend: encode ListStates response: %w", err)
		}
...
func (s *awsIaCServer) DeleteState(ctx context.Context, req *pb.DeleteStateRequest) (*pb.DeleteStateResponse, error) {
	store, err := s.stateBackend.resolveStore()
	if err != nil {
		return nil, err
	}
...
func (s *awsIaCServer) Lock(ctx context.Context, req *pb.LockRequest) (*pb.LockResponse, error) {
	store, err := s.stateBackend.resolveStore()
	if err != nil {
		return nil, err
	}
...
func (s *awsIaCServer) Unlock(ctx context.Context, req *pb.UnlockRequest) (*pb.UnlockResponse, error) {
	store, err := s.stateBackend.resolveStore()
	if err != nil {
		return nil, err
	}
...
func (s *awsIaCServer) ListBackendNames(_ context.Context, _ *pb.ListBackendNamesRequest) (*pb.ListBackendNamesResponse, error) {
	return &pb.ListBackendNamesResponse{BackendNames: []string{awsStateBackendName}}, nil
}
